...
def _normalize_observance_name(name: str) -> str:
    normalized = name.lower()
    normalized = normalized.replace("women's", "women")
    normalized = normalized.replace("womens", "women")
    normalized = normalized.replace("teachers'", "teachers")
    normalized = normalized.replace("teacher's", "teachers")
    normalized = re.sub(r"[^a-z0-9]+", " ", normalized)
    return normalized.strip()


def _find_curated_entry(name: str) -> Optional[tuple[str, Dict[str, Any]]]:
    observed = _normalize_observance_name(name)
    for curated_name, metadata in CURATED_OBSERVANCES.items():
        curated = _normalize_observance_name(curated_name)
